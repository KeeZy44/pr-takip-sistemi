...
# --- ASLA SİLİNMEYECEK MUTLAK KÖK KLASÖR YOLU ---
# Veritabanını Streamlit'in kod klasöründen çıkartıp, sunucunun silinmez güvenli bölgesine gömüyoruz.
DB_PATH = '/tmp/pr_yonetim_yedekli.db'
...
def link_kaydet(campaign, sayfa, link, ucret, durum='Bekliyor'):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    tarih = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    c.execute("INSERT INTO pr_kayitlar (tarih, kampanya_adi, sayfa_adi, video_linki, ucret, durum) VALUES (?, ?, ?, ?, ?, ?)",
              (tarih, campaign, sayfa.strip(), link.strip(), ucret, durum))
    conn.commit()
    conn.close()
...
def kayit_guncelle(kayit_id, yeni_campanya):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE pr_kayitlar SET kampanya_adi = ? WHERE id = ?", (yeni_campanya, kayit_id))
    conn.commit()
    conn.close()
...
with tab_link_ekle:
    ...
    if not mevcut_kampanyalar_aktif:
        st.warning("Henüz aktif ve girişlere açık bir PR kampanyası bulunmuyor. Patron iseniz şifre girip kampanya açın veya dondurulanı aktif edin.")
    else:
            ...
            with st.form("sayfa_giriş_formu", clear_on_submit=False):
                ...
                if submit_butonu:
                    if sayfa_adi and video_linki and calisilan_ucret > 0:
                        ...
                        if eski_kayit: st.error("❌ UYARI: Bu sayfa zaten link gönderdi!")
                        else:
        ...
        with col_guncelle:
            ...
            if g_sayfa:
                ...
                if mevcut_link_verisi:
                    st.warning(f"📋 Kayıtlı linkiniz:\n{mevcut_link_verisi[0]}")
                    yeni_link_girdisi = st.text_input("👉 Yeni Link:", key="g_yeni_link")
                    if st.button("💾 Kaydet"):
...
if is_patron:
    with tab_patron_paneli:
        ...
        st.markdown("---")
        st.subheader("👤 Sayfa Bazlı Toplu Ödeme Kapatma")
        mevcut_sayfalar = sayfaları_getir()
        if mevcut_sayfalar:
            secilen_toplu_sayfa = st.selectbox("Sayfa Seçin:", mevcut_sayfalar)
            if secilen_toplu_sayfa:
                sayfa_df = df_genel[df_genel['sayfa_adi'] == secilen_toplu_sayfa]
                st.metric(label="🔴 TOPLAM BORÇ", value=f"{sayfa_df[sayfa_df['durum'] == 'Bekliyor']['ucret'].sum():,.2f} TL")
                if st.button("🚀 Sayfanın Tüm Borçlarını Kapat"):
        ...
        if mevcut_kampanyalar_hepsi:
            izlenecek_campaign = st.selectbox("Kampanya Seçin:", mevcut_kampanyalar_hepsi)
            conn = sqlite3.connect(DB_PATH)
            ...
            if not df.empty:
                ...
                st.text_area("Linkler:", value="\n".join(df['Video Linki'].tolist()), height=150)
                df['secim_metni_odeme'] = df['Sayfa'] + " | " + df['Ücret (TL)'].astype(str) + " TL [" + df['Durum'] + "]"
                secili_odeme_row = st.selectbox("Satır Seç:", df['secim_metni_odeme'].tolist())
                if secili_odeme_row:
                    secili_id = int(df[df['secim_metni_odeme'] == secili_row]['id'].values[0] if 'secili_row' in locals() else df[df['secim_metni_odeme'] == secili_odeme_row]['id'].values[0])
                    if st.button("Tekil Ödeme Durumu Değiştir"):
                        curr = df[df['id'] == secili_id]['Durum'].values[0]
                        odeme_durumu_degistir(secili_id, 'Ödendi' if curr=='Bekliyor' else 'Bekliyor')
                        st.rerun()
